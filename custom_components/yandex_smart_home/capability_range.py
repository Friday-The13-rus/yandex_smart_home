...
from homeassistant.components import climate, cover, fan, humidifier, light, media_player, water_heater, valve
...
class ValvePositionCapability(StateRangeCapability):
    ...
    def supported(self) -> bool:
        """Test if the capability is supported."""
        return self.state.domain == valve.DOMAIN and bool(
            self._state_features & valve.ValveEntityFeature.SET_POSITION
        )
